...
```{r}
var.label <- c("q1","q4","q11","q1719", "q38","q39","q40","q41")
showtext_auto()
...
my_scatter <- function(data, x, y, group,
  ...
  if (!all(required_cols %in% names(data))) {
    stop("Some specified columns are not in the data.")
  }
...
## {background-image=background.jpeg}
### 觀察不同年齡下，抵制者與不抵制者的平均看見網路攻擊頻率

```{r}
my_scatter(DB.csv, x = 'q2', y = 'q22', group = 'q28_YN',
           title = "不同年齡的網路環境好壞", 
           xlab = "年齡", 
           ylab = "平均看見網路攻擊頻繁程度",
           legend.lab = "抵制者:1\n未抵制者:0")
...
## {background-image=background.jpeg}
### 觀察不同年齡下，抵制者與不抵制者的自己網路攻擊頻率

```{r}
# 連續型年齡
my_scatter(DB.csv, x = 'q2', y = 'q23', group = 'q28_YN',
           title = "不同年齡的自己網路攻擊頻率", 
           xlab = "年齡", 
           ylab = "自己的網路攻擊頻繁程度",
           legend.lab = "抵制者:1\n未抵制者:0")
...
## {background-image=background.jpeg}
### 觀察不同年齡下，抵制者與不抵制者的心理幸福感

```{r}
# 連續型年齡
my_scatter(DB.csv, x = 'q2', y = 'q38', group = 'q28_YN',
           title = "不同年齡的心理幸福感", 
           xlab = "年齡", 
           ylab = "心理幸福感",
           legend.lab = "抵制者:1\n未抵制者:0")
...
split.fun <- function(x, labs, digits, varlen, faclen)
{
    ...
    labs <- sub("q22", "他人攻擊行為的頻率", labs)
    labs <- sub("q2",      "年齡", labs)
    labs   <- sub("q40",    "國民黨喜好程度", labs)
    labs   <- sub("q7",    "娛樂上網分鐘", labs)
    labs   <- sub("q26",    "不文明留言影響力", labs)
    labs   <- sub("q10",    "使用可討論名人之\n社群媒體數量", labs)
...
:::{.content-visible style="position: absolute; top: 40%; left: 0; transform: translateY(-50%); text-align: left; font-size: 25px; line-height:3;"}
我們認為抵制程度能用「抵制效度」、「抵制傷害程度」、「抵制規模」這三個問題來衡量，

因此我們將這些變數設為反應變數、其他的變數設為解釋變數，去進行典型相關分析。

過程中發現有11個變數的值和第一典型負荷量小於等於0.2，因此將這些變數去除後，再做一次典型相關分析。
:::
...
:::{.content-visible style="font-size: 25px;"}
典型相關係數: 0.556     自我相關係數(解釋變數): 0.778     自我相關係數(反應變數): 0.318
:::
...
## 推論與討論 {background-image="background.jpeg"}
...
2.  **影音平台使用**<br/>
使用多種平台助於接觸不同觀點，降低煽動效應與參與抵制的可能性。<br/>
3.  **網路行為**<br/>
負面環境助長群體的攻擊性，進而提升參與抵制的可能性與抵制程度；
媒體識讀使得抵制行為產生極化。<br/>
4.  **政黨好感度及意識型態**<br/>
意識型態上偏好台獨者，更傾向參與抵制，抵制程度也較高。<br/>
5.  **抵制行為的重要性**<br/>
抵制行為之於個人越重要，代表越重視抵制成效，也加劇抵制的程度。<br/>
