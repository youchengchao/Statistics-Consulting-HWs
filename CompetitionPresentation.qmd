...
tempDF <- DB.csv %>%
  group_by(q2,q28_YN) %>% 
  summarise(q22_mean = mean(q22), q22_sd = sd(q22))

ggplot(data = tempDF)+
  geom_line(aes(y=q22_mean,
                x=q2, 
                color=factor(q28_YN)),
            show.legend = FALSE) +
  geom_ribbon(aes(x = q2,
                  ymin = q22_mean - q22_sd,
                  ymax = q22_mean + q22_sd,
                  fill = factor(q28_YN)),
              alpha = 0.2) + 
  labs(title = "不同年齡的網路環境好壞",
       x = "年齡",
       y = "平均看見網路攻擊頻繁程度",
       fill = "抵制者:1\n未抵制者:0")
...
# 年齡分層
tempDF <- DB.csv %>%
  group_by(q2rr,q28_YN) %>% 
  summarise(q22_mean = mean(q22), q22_sd = sd(q22))

ggplot(data = tempDF, aes(y=q22_mean,x=q2rr, group=factor(q28_YN)))+
  geom_point()+
  geom_line(aes(color=factor(q28_YN)),show.legend = FALSE) +
  geom_ribbon(aes(x = q2rr,
                  ymin = q22_mean - q22_sd,
                  ymax = q22_mean + q22_sd,
                  fill = factor(q28_YN)),
              alpha = 0.2) + 
...
tempDF <- DB.csv %>%
  group_by(q2,q28_YN) %>% 
  summarise(q23_mean = mean(q23),
            q23_sd = sd(q23), .groups = "drop")

ggplot(data = tempDF)+
  geom_line(aes(y=q23_mean,
                x=q2, 
                color=factor(q28_YN)),
            show.legend = FALSE) +
  geom_ribbon(aes(x = q2,
                  ymin = q23_mean - q23_sd,
                  ymax = q23_mean + q23_sd,
                  fill = factor(q28_YN)),
              alpha = 0.2) + 
  labs(title = "不同年齡的自己網路攻擊頻率",
       x = "年齡",
       y = "自己的網路攻擊頻繁程度",
       fill = "抵制者:1\n未抵制者:0")
...
ggplot(data = tempDF, aes(y=q23_mean,x=q2rr,group=factor(q28_YN)))+
  geom_point(show.legend = FALSE) +
  geom_line(aes(color=factor(q28_YN)))+
  geom_ribbon(aes(x = q2rr,
                  ymin = q23_mean - q23_sd,
                  ymax = q23_mean + q23_sd,
                  fill = factor(q28_YN)),
...
tempDF <- DB.csv %>%
  group_by(q2,q28_YN) %>% 
  summarise(q38_mean = mean(q38),
            q38_sd = sd(q38), .groups = "drop")

ggplot(data = tempDF)+
  geom_line(aes(y=q38_mean,
                x=q2, 
                color=factor(q28_YN)),
            show.legend = FALSE) +
  geom_ribbon(aes(x = q2,
                  ymin = q38_mean - q38_sd,
                  ymax = q38_mean + q38_sd,
                  fill = factor(q28_YN)),
              alpha = 0.2) + 
  labs(title = "不同年齡的心理幸福感",
       x = "年齡",
       y = "心理幸福感",
       fill = "抵制者:1\n未抵制者:0")
...
ggplot(data = tempDF, aes(y=q38_mean,x=q2rr,group=factor(q28_YN)))+
  geom_point(show.legend = FALSE) +
  geom_line(aes(color=factor(q28_YN)))+
  geom_ribbon(aes(x = q2rr,
                  ymin = q38_mean - q38_sd,
