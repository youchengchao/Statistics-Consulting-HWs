...
```{r}
ggplot(data, aes(x = volume, y = price, color =factor(cut))) +
  geom_point(alpha = 0.5) +
...
而model.2的R-squared = 0.9041，VIF檢查後各變數也無異常

接著我們把資料集切割成訓練集70%和測試集30%來對此模型評估預測表現
...
model <- lm(price ~ cut + color + clarity + table + volume, data = train_data)
summary(model)
...
經過測試後，這個模型R-square = 0.9053，RMSE = 5348.818
