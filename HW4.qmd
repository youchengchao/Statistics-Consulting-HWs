...
# 請提供3號候選人的競選策略(需在何地、對何人進行拉票)

# 以V4回答出候選人人數來評估受訪者「政治熱衷程度」，建立合適統計模型分析該變數並說明使用該方法的原因。

# 請建立3號候選人支持率(具資料不平衡特性)的預測模式，請說明如何處理資料不平衡的問題。
...
count5.total <- unlist(lapply(1:11,function(x){
  if(x==11) return(sum(is.na(pollcsv$v5)))
  else return(sum(pollcsv$v5[!is.na(pollcsv$v5)]==x))
} ))
#計算分區支持度(北區中西區)
North <-subset(pollcsv,pollcsv$v1==1)
MidWest <-subset(pollcsv,pollcsv$v1==2)
n1 <- dim(North)[1]
n2 <- dim(MidWest)[1]
count5.North <- unlist(lapply(1:11,function(x){
  if(x==11) return(sum(is.na(North$v5)))
  else return(sum(North$v5[!is.na(North$v5)]==x))
} ))
count5.MidWest <- unlist(lapply(1:11,function(x){
  if(x==11) return(sum(is.na(MidWest$v5)))
  else return(sum(MidWest$v5[!is.na(MidWest$v5)]==x))
} ))
#計算性別支持度
male <-subset(pollcsv,pollcsv$v8==1)
female <-subset(pollcsv,pollcsv$v8==2)
n.male <- dim(male)[1]
n.female <- dim(female)[1]
count5.male <- unlist(lapply(1:11,function(x){
  if(x==11) return(sum(is.na(male$v8)))
  else return(sum(male$v8[!is.na(male$v8)]==x))
} ))
count5.female <- unlist(lapply(1:11,function(x){
  if(x==11) return(sum(is.na(female$v8)))
  else return(sum(female$v8[!is.na(female$v8)]==x))
} ))
#計算年齡支持度
