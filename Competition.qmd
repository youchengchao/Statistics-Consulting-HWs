...
第八題: 大部分的人都有透過網路接觸名人的資訊或討論(只有四個人沒有)，所以決定移除。

第九題: 即時通訊軟體多為聯繫熟人或工作上使用，較難顯現是否有突破同溫層的現象，所以決定移除。

疫情相關題目: 關心的題目(28,29)時間範圍較廣，並不只局限於疫情期間，所以決定移除。

接著在對一些題目進行細部的選項討論:

第十題: 改成"使用幾個與yt名人討論相關的社群媒體"，因為有些社群媒體不會造成抵制名人行為，例如:Pinterest,Linkedin,+其他類Pixiv,Mobile01,Komica,MeWe 跟名人相關的討論比較少，所以決定簡化選項; 巴哈姆特, 巴哈姆特場外休憩區 兩個則要計算。

第十一題: 改成"有無使用YT, Twitch,或bilibili"(1:有使用,0:沒有使用)，原因與第十題類似。
...
| q17_01    | 是否參與過是騙人、不會對人造成傷害的網路惡搞   |1:是,0:否|
| q17_02    | 是否參與過不是騙人的不會對人造成傷害的網路惡搞 |1:是,0:否|
| q19_01    | 是否參與過是騙人、會對人造成傷害的網路惡搞     |1:是,0:否|
| q19_02    | 是否參與過不是騙人的會對人造成傷害的網路惡搞   |1:是,0:否|
| q20       | 主動激化傾向                       |       |
| q22       | 他人攻擊傾向|                              |
| q23       | 自己攻擊傾向|                              |
| q24       | 回聲室效應 |                                 |
...
首先對其他類進行歸類: 28題的其他類分到前三類或是設0，29題
...
因為想要做的方向有兩個: "甚麼原因會造成有抵制行為?", "甚麼原因會影響抵制行為的程度"，所以建立q28_5
...
1: 有惡搞行為(17,19的四題至少選一)
0: 沒有惡搞(17,19的四題完全都沒選)
...
DB.csv$q1719.label <- apply(
  DB.csv[,match("q17_01",colnames(DB.csv)):match("q19_02",colnames(DB.csv))],
  MARGIN = 1,
  function(row){
    return(paste0(row,collapse = ""))
  })
unique(DB.csv$q1719.label)
DB.csv$q1719.label <- ifelse(DB.csv$q1719.label=="0000", 0, 1)
```
...
#| eval: false
counts_17_19 <- DB.csv %>%
  group_by(q28_5, q1719.label) %>%
  summarise(count = n())
#圖
ggplot(counts_17_19, aes(x = as.factor(q28_5), y = q1719.label, fill = count)) +
  geom_tile() +
  scale_fill_gradient(low = "lightblue", high = "red") +
  geom_text(label=counts_17_19$count,size = 3, color = "black", fontface = "bold")+
...
```{r}
glm_log <- glm(as.factor(q28_5)~q1+as.factor(q2)+q3+as.factor(q4)+q5_1+q6+q7+q10+as.factor(q11)+q1719.label+as.factor(q20_01_1)+as.factor(q20_02_1)+q22_total+q23_total+q24_total+q25_total+q26_total+as.factor(q27_1), family = binomial(), data = DB.csv,weights = weight)
...
tree_model <- rpart(as.factor(q28_5) ~  q1 + q2 + as.factor(q4) + q5_1 +q6 + q7 + as.factor(q10) + as.factor(q11) + q1719.label + as.factor(q20_01_1)+q20_02_1 + q22_total + q23_total + q24_total + q25_total + q26_total + q27_1,data = DB.csv, method = "class",weights = weight)
...
#pca
pca<-prcomp(DB.csv[,c('q1','q4','q5_1','q6','q7','q10','q11','q1719.label','q20_01_1','q20_02_1','q22_total','q23_total','q24_total','q25_total','q26_total','q27_1')],center = T)
#做Scree plot 可以發現到3斜率都還很大
screeplot(pca,type = 'line', main= "Fig 1: Scree plot of explained variabilities in PCA")
rec_pc <- as.data.frame(pca$x[,1:3])
#將數字的label 加進去pc1 跟 pc2 的dataset
rec_pc$label<-DB.csv$q28_5
...
pca_rotation2<-pca_rotation_longs%>%
  filter(PCA=='PC2')
pca_rotation3<-pca_rotation_longs%>%
  filter(PCA=='PC3')
...
ggplot(rec_pc,aes(x = PC2, y = PC3,colour = label))+
  geom_point()+ labs(title = 'Fig 5: PC1 vs. PC2')+
  theme(plot.title = element_text(hjust = 0.5), legend.position = 'right')

# glmnet and xgboost
library(glmnet)
x <- model.matrix(as.factor(q28_5) ~ q1+q2+q3+q4+q5_1+q6+q7+q10+q11+q1719.label+q20_01_1+q20_02_1+q22_total+q23_total+q24_total+q25_total+q26_total+q27_1, data = DB.csv)[, -1]
y <- as.factor(DB.csv$q28_5)
...
```{r}
#| eval: false
Barplot.p(din=myCount_q28(DB.csv, "q1719.label"))
