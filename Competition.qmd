...
```{r}
#DataExplorer::plot_correlation(DB.csv[,-41], cor_args = list(method="spearman"))
```
...
# library(ggpubr)
# library(magrittr)
# tSNE.2d <- function(X, label_vector
#                     seed=123,
#                     theta = 0.5,
#                     perplexity = 30,
#                     size = 1.5, num_threads = 1L ){
...
ggplot(df, aes(x = reorder(Name, -abs(Value)), y = abs(Value), fill = as.factor(Positive))) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(abs(Value), 2)), 
            vjust = -0.5, size = 3) +  # 在條形圖上顯示絕對值
  scale_fill_manual(values = c("0" = "red", "1" = "blue"), 
                    labels = c("Negative", "Positive")) +
  labs(x = "Question", y = "Absolute Loadings", fill = "Sign") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle("Bar Plot of Absolute Loadings(典型負荷量)")
