...
檢視資料後發現大部分人都會透過網路接觸名人的資訊(只有四個山頂洞人)，
...
Pinterest,Linkedin,Pixiv,Mobile01,Komica,MeWe 跟yt名人相關的討論比較無關;
...
#移除q8,q9
DB.csv[,which(colnames(DB.csv)=="q8_1"):which(colnames(DB.csv)=="q8_88_text")] <- NULL
DB.csv[,match("q9_1",colnames(DB.csv)):match("q9_90",colnames(DB.csv))] <- NULL

#移除第十、十一題
unique(DB.csv$q11_88_text)
...
因此可以將28題的複選題轉成多類別單選(C4取0~C4取4): 八種組合
...
```{r}
which(colnames(DB.csv)=="weight")
which(colnames(DB.csv)=="q28_5")
```
